...
static UAModel3* singleton = nil;

+(UAModel3 *)defaultUAModel3{
    static dispatch_once_t defaultUAModel3onceToken;
    dispatch_once(&defaultUAModel3onceToken, ^{
        if(singleton == nil){
            singleton=[[UAModel3 alloc] init];
//            if([[NSFileManager defaultManager] fileExistsAtPath:[UAModel3 dataFilePath]]){
////                singleton=[NSKeyedUnarchiver unarchiveObjectWithFile: [UAModel3 dataFilePath] ];
//            }
//            else{
//                
//            }
...
-(id)init
{
    if(self = [super init]){
        self.mobiletype = @"1";
        self.sdk_imei = [CmmobiClickAgent getOpenUDID];
        self.channel_number = @"looklook";
        self.basestationinfo = @"";
        
        self.gps = @"";
        self.clientversion = PRODUCT_VERSION;
        self.siteid = @"";
        self.cdr = @"";
