...
- (BOOL)canAcceptBytes:(CFSocketRef)sockRef
{
	...
	if(theNativeSocket == 0)
	{
		debug_NSLog(@"Error - Could not get CFSocketNativeHandle from CFSocketRef");
		return NO;
	} 
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketWriteCallBack:
			...
			break;
		default:
			debug_NSLog (@"AsyncUdpSocket %p received unexpected CFSocketCallBackType %lu.", self, (unsigned long)type);
			break;
	}
}
